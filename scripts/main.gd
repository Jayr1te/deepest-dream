...
func _physics_process(delta: float) -> void:
	...
	oxyBar.offset_right = 100 * (log(Res.oxyMax) / log(10))
	oxyBar.get_child(1).offset_left = 100 * (log(Res.oxyMax) / log(10))
	oxyBar.get_child(1).offset_right = 100 * (log(Res.oxyMax) / log(10))
	oxyBar.max_value = Res.oxyMax
	oxyBar.value = Res.oxy
	
	oxyBar.get_child(0).text = str(snapped(Res.oxy,1))
	oxyBar.get_child(1).text = str(snapped(Res.oxyMax,1))
	oxyBar.get_child(2).text = str(snapped(depth,1)) + "m"

	if Res.mem > 0 and memUnlocked == false:
		memUnlocked = true
	
	$CanvasLayer/MemIcon.visible = memUnlocked
	
	$CanvasLayer/MemIcon/MemNumber.text = str(snapped(Res.mem,1))
	...
	if diving:
		...
		if Res.oxy < Res.oxyMax:
			if Res.oxy < 0:
				Res.oxy = 0
			Res.oxy += Res.oxyMax * delta
			if Res.oxy > Res.oxyMax:
				Res.oxy = Res.oxyMax
		depth = 0
		explore = 0
